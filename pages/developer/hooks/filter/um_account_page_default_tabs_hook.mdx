import { Tabs, Tab, Callout } from 'nextra-theme-docs' 
import Link from 'next/link'

### Filter Hook    
    
```
apply_filters( 'um_account_page_default_tabs_hook', $tabs );
```

#### Description
    <div class="description">
        Account Page Tabs
 
    </div>

#### Parameters
            
<table className="table-auto mt-2 border dark:border-gray-500 p-5 text-sm text-left">
<thead>
<tr>
<th  className="px-3">Name</th>
<th  className="px-3">Type</th>
<th  className="px-3">Description</th>
</tr>
</thead>
<tbody>
               <tr >
               <td className="px-3 py-2">`$tabs`</td>
               <td className="px-3 py-2">array</td>
               <td className="px-3 py-2 text-sm">Account Page Tabs</td>
               </tr>
</tbody>
</table>

#### Since
<div class="description">
2.0
</div>

#### Source
  - <Link href="/developer/files/includes-core-class-account#source-view.150">ultimatemember/includes/core/class-account.php</Link> <Link href="/developer/files/includes-core-class-account#source-view.150">line 150</Link>



#### Usage Example

<Tabs items={['PHP']}>
    <Tab>
    ```php
    add_filter( 'um_account_page_default_tabs_hook', 'my_account_page_default_tabs', 10, 1 );
 function my_account_page_default_tabs( $tabs ) {
     // your code here
     return $tabs;
 }
    ```
    </Tab>
</Tabs>