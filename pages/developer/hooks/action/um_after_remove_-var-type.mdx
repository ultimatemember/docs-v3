import { Tabs, Tab, Callout } from 'nextra-theme-docs' 
import Link from 'next/link'

### Action Hook    
    
```
do_action( "um_after_remove_{$type}", $user_id );
```

#### Description
    <div class="description">
        Make some actions after remove file
 
    </div>

#### Parameters
            
<table className="table-auto mt-2 border dark:border-gray-500 p-5 text-sm text-left">
<thead>
<tr>
<th  className="px-3">Name</th>
<th  className="px-3">Type</th>
<th  className="px-3">Description</th>
</tr>
</thead>
<tbody>
               <tr >
               <td className="px-3 py-2">`$user_id`</td>
               <td className="px-3 py-2">int</td>
               <td className="px-3 py-2 text-sm">User ID</td>
               </tr>
</tbody>
</table>

#### Since
<div class="description">
2.0
</div>

#### Source
  - <Link href="/developer/files/includes-core-class-files#source-view.1087">ultimatemember/includes/core/class-files.php</Link> <Link href="/developer/files/includes-core-class-files#source-view.1087">line 1087</Link>



#### Usage Example

<Tabs items={['PHP']}>
    <Tab>
    ```php
    add_action( 'um_after_remove_{$type}', 'my_after_remove_file', 10, 1 );
 function my_after_remove_file( $user_id ) {
     // your code here
 }
    ```
    </Tab>
</Tabs>