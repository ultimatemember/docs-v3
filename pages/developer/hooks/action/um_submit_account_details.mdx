import { Tabs, Tab, Callout } from 'nextra-theme-docs' 
import Link from 'next/link'

### Action Hook    
    
```
do_action( 'um_submit_account_details', UM()->form()->post_form );
```

#### Description
    <div class="description">
        On success account submit
 
    </div>

#### Parameters
            
<table className="table-auto mt-2 border dark:border-gray-500 p-5 text-sm text-left">
<thead>
<tr>
<th  className="px-3">Name</th>
<th  className="px-3">Type</th>
<th  className="px-3">Description</th>
</tr>
</thead>
<tbody>
               <tr >
               <td className="px-3 py-2">`$submitted`</td>
               <td className="px-3 py-2">array</td>
               <td className="px-3 py-2 text-sm">Account Page Submitted data</td>
               </tr>
</tbody>
</table>

#### Since
<div class="description">
2.0
</div>

#### Source
  - <Link href="/developer/files/includes-core-class-account#source-view.422">ultimatemember/includes/core/class-account.php</Link> <Link href="/developer/files/includes-core-class-account#source-view.422">line 422</Link>



#### Usage Example

<Tabs items={['PHP']}>
    <Tab>
    ```php
    add_action( 'um_submit_account_details', 'my_submit_account_details', 10, 1 );
 function my_submit_account_details( $submitted ) {
     // your code here
 }
    ```
    </Tab>
</Tabs>